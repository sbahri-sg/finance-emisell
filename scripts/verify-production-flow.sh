...
curl -fsS -b "$cookie_file" "$base/api/bootstrap" | jq -e --arg deposit "$deposit_id" '(.accounts[]|select(.id==$deposit)|.balance|tonumber)==1700000 and (.accounts[]|select(.id==$deposit)|.monthlyUsage|tonumber)==300000' >/dev/null
