...
cash_transfer_id=$(curl -fsS -b "$cookie_file" -H 'Content-Type: application/json' -d '{"name":"Kas Transfer Test","institution":"Internal","kind":"cash","currency":"IDR","openingBalance":0,"color":"#225c55"}' "$base/api/accounts" | jq -er '.id')
internal_transfer_payload=$(jq -nc --arg source "$account_id" --arg destination "$cash_transfer_id" '{transactionDate:"2026-08-11",sourceAccountId:$source,destinationAccountId:$destination,amount:500000,reference:"TRANSFER-TEST-001",note:"Isi kas kecil test"}')
internal_transfer_id=$(curl -fsS -b "$cookie_file" -H 'Content-Type: application/json' -d "$internal_transfer_payload" "$base/api/transfers" | jq -er '.id')
curl -fsS -b "$cookie_file" "$base/api/bootstrap" | jq -e --arg source "$account_id" --arg destination "$cash_transfer_id" --arg transaction "$internal_transfer_id" '(.accounts[]|select(.id==$source)|.balance|tonumber)==9500000 and (.accounts[]|select(.id==$destination)|.balance|tonumber)==500000 and (.transactions[]|select(.id==$transaction)|.kind)=="transfer" and (.transactions[]|select(.id==$transaction)|.amount|tonumber)==-500000 and (.transactions[]|select(.id==$transaction)|.accountId)==$source' >/dev/null
curl -fsS -b "$cookie_file" "$base/api/reports?month=2026-08" | jq -e '(.summary.income|tonumber)==0 and (.summary.expense|tonumber)==0' >/dev/null
[ "$(curl -sS -o /dev/null -w '%{http_code}' -b "$cookie_file" -H 'Content-Type: application/json' -d "$internal_transfer_payload" "$base/api/transfers")" = 409 ]
curl -fsS -b "$cookie_file" -H 'Content-Type: application/json' -d '{"transactionDate":"2026-08-11","reason":"Batalkan transfer internal uji"}' "$base/api/transactions/$internal_transfer_id/reverse" | jq -er '.id' >/dev/null
curl -fsS -b "$cookie_file" "$base/api/bootstrap" | jq -e --arg source "$account_id" --arg destination "$cash_transfer_id" '(.accounts[]|select(.id==$source)|.balance|tonumber)==10000000 and (.accounts[]|select(.id==$destination)|.balance|tonumber)==0' >/dev/null
curl -fsS -b "$cookie_file" -X DELETE "$base/api/accounts/$cash_transfer_id" | jq -e '.ok==true' >/dev/null
...
vcc_bill_id=$(curl -fsS -b "$cookie_file" -H 'Content-Type: application/json' -d '{"vendor":"DigitalOcean Test","description":"Server via VCC","dueDate":"2026-08-15","unitPrice":75000,"quantity":1,"paymentMethod":"vcc","currency":"IDR","recurrence":"once","owner":"IT","autoRenew":true,"reminderDays":[14,7,1]}' "$base/api/bills" | jq -er '.id')
vcc_bill_payload=$(jq -nc --arg account "$deposit_id" --arg category "$category_id" '{transactionDate:"2026-08-11",accountId:$account,amount:75000,reference:"VCC-BILL-TEST-001",budgetCategoryId:$category}')
vcc_bill_transaction_id=$(curl -fsS -b "$cookie_file" -H 'Content-Type: application/json' -d "$vcc_bill_payload" "$base/api/bills/$vcc_bill_id/pay" | jq -er '.transactionId')
curl -fsS -b "$cookie_file" "$base/api/bootstrap" | jq -e --arg bill "$vcc_bill_id" --arg transaction "$vcc_bill_transaction_id" --arg deposit "$deposit_id" '(.bills[]|select(.id==$bill)|.status)=="paid" and (.transactions[]|select(.id==$transaction)|.kind)=="deposit_usage" and (.transactions[]|select(.id==$transaction)|.paymentMethod)=="vcc" and (.transactions[]|select(.id==$transaction)|.accountId)==$deposit and (.accounts[]|select(.id==$deposit)|.balance|tonumber)==1625000' >/dev/null
curl -fsS -b "$cookie_file" -X DELETE "$base/api/bills/$vcc_bill_id" | jq -e '.ok==true and .archived==true' >/dev/null
curl -fsS -b "$cookie_file" -H 'Content-Type: application/json' -d '{"transactionDate":"2026-08-11","reason":"Batalkan pembayaran VCC untuk uji ringkasan"}' "$base/api/transactions/$vcc_bill_transaction_id/reverse" | jq -er '.id' >/dev/null
curl -fsS -b "$cookie_file" "$base/api/bootstrap" | jq -e --arg deposit "$deposit_id" --arg transaction "$vcc_bill_transaction_id" '(.accounts[]|select(.id==$deposit)|.balance|tonumber)==1700000 and (.accounts[]|select(.id==$deposit)|.monthlyUsage|tonumber)==300000 and ([.depositActivities[]|select(.id==$transaction and .kind=="deposit_usage" and .reversed==true)]|length)==1 and ([.depositActivities[]|select(.kind=="reversal" and .reversalOf==$transaction and .amount>0)]|length)==1' >/dev/null
