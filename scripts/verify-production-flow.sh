...
curl -fsS -b "$cookie_file" "$base/api/bootstrap" | jq -e --arg account "$account_id" --arg deposit "$deposit_id" '(.accounts[]|select(.id==$account)|.balance|tonumber)==7475000 and (.accounts[]|select(.id==$deposit)|.balance|tonumber)==1625000 and ([.transactions[]|select(.description=="Income sebelum edit" or .description=="Expense sebelum edit")]|length)==0 and ([.transactions[]|select(.description=="Income setelah edit" or .description=="Expense setelah edit")]|length)==2 and ([.transactions[]|select(.description=="Expense setelah edit")|.budgetItemName][0])=="Galon" and ([.bills[]|select(.vendor=="Cloud Test" or .vendor=="DigitalOcean Test")]|length)==0' >/dev/null
curl -fsS -b "$cookie_file" "$base/api/reports?month=2026-08" | jq -e '(.summary.income|tonumber)==700000 and (.summary.expense|tonumber)==1600000 and (.summary.depositBalance|tonumber)==1625000' >/dev/null
