...
selow_import_payload=$(jq -nc --arg source "$import_source_id" --arg category "$category_id" '{sourceAccountId:$source,statementBalance:50000,rows:[{transactionDate:"2026-08-12",transactionTime:"17:00:00",note:"Top-up manual",amount:100000},{transactionDate:"2026-08-12",transactionTime:"18:00:00",note:"FACEBK *MATCH Dublin IE",amount:-100000,budgetCategoryId:$category},{transactionDate:"2026-08-13",transactionTime:"10:00:00",amount:200000},{transactionDate:"2026-08-13",transactionTime:"11:00:00",note:"FACEBK *NEW Dublin IE",amount:-200000,budgetCategoryId:$category}]}')
...
curl -fsS -b "$cookie_file" "$base/api/bootstrap" | jq -e --arg deposit "$import_deposit_id" '(.accounts[]|select(.id==$deposit)|.balance|tonumber)==50000 and (.accounts[]|select(.id==$deposit)|.reconciled)==true' >/dev/null
