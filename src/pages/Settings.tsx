...
const categoryReferenceCount=(category:ExpenseCategoryLabel)=>(category.transactionCount||0)+(category.historyCount||0)+(category.budgetCount||0)
...
export function Settings(){
  ...
  async function deleteExpenseCategory(){if(!deleteCategoryTarget)return;const used=categoryReferenceCount(deleteCategoryTarget)>0;const ok=await request(`/api/expense-categories/${deleteCategoryTarget.id}`,'DELETE',used?{replacementCategoryId}:{});if(ok){setDeleteCategoryTarget(null);setReplacementCategoryId('');await load()}}
  ...
  return <>
    ...
    {deleteCategoryTarget&&(()=>{const used=categoryReferenceCount(deleteCategoryTarget)>0;const replacements=data.expenseCategories.filter(category=>category.id!==deleteCategoryTarget.id&&category.active);return <Modal title={used?'Gabungkan & hapus kategori?':'Hapus kategori?'} description="Konfirmasi diperlukan sebelum label kategori dihapus" onClose={()=>!saving&&setDeleteCategoryTarget(null)}>
      {error&&<div className="auth-error delete-confirmation-error">{error}</div>}
      <div className="delete-confirmation"><span className="delete-confirmation-icon"><Trash2 size={25}/></span><div><strong>{deleteCategoryTarget.name}</strong><p>{used?`${deleteCategoryTarget.transactionCount||0} transaksi aktif, ${deleteCategoryTarget.historyCount||0} histori, dan ${deleteCategoryTarget.budgetCount||0} pos RAB akan dipindahkan ke kategori pengganti.`:'Label belum digunakan dan dapat dihapus langsung.'}</p></div></div>
      {used&&<label className="category-merge-field"><span>Kategori pengganti</span><select value={replacementCategoryId} onChange={event=>setReplacementCategoryId(event.target.value)} disabled={saving}><option value="">Pilih kategori tujuan</option>{replacements.map(category=><option key={category.id} value={category.id}>{category.name}</option>)}</select><small>Histori laporan akan memakai label kategori pengganti.</small></label>}
      <div className="delete-confirmation-note"><AlertTriangle size={17}/><span>{used?'Nominal transaksi, jurnal, dan saldo rekening tidak berubah. Hanya pengelompokan kategori yang dipindahkan.':'Penghapusan label tidak dapat dibatalkan setelah disimpan.'}</span></div>
      <div className="modal-actions delete-confirmation-actions"><Button variant="secondary" onClick={()=>setDeleteCategoryTarget(null)} disabled={saving}>Batal</Button><Button variant="danger" onClick={()=>void deleteExpenseCategory()} disabled={saving||(used&&!replacementCategoryId)}><Trash2 size={15}/>{saving?'Memproses…':used?'Gabungkan & hapus':'Hapus kategori'}</Button></div>
    </Modal>})()}
  </>
}
