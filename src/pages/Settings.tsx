...
import { Bell, Building2, CheckCircle2, Database, Download, FileDown, KeyRound, Landmark, LockKeyhole, LogOut, Pencil, Plus, Save, ShieldCheck, SlidersHorizontal, Tags, Trash2, Users } from 'lucide-react'
...
export function Settings(){
  ...
  async function deleteExpenseCategory(category:ExpenseCategoryLabel){if(!window.confirm(`Hapus kategori “${category.name}” secara permanen?`))return;if(await request(`/api/expense-categories/${category.id}`,'DELETE'))await load()}
  ...
  return <>
    ...
    <div className="settings-layout"><aside className="settings-nav" aria-label="Menu pengaturan">
      ...
      {active==='finance'&&<><Card><SettingHeading title="Preferensi keuangan" description="Standar pencatatan yang dipakai seluruh tim." locked={locked}/><form className="form-grid" action={saveFinance}>
        ...
        <div className="expense-category-list">{data.expenseCategories.map(category=>editingCategoryId===category.id?<form className="expense-category-edit" action={saveExpenseCategory} key={category.id}><input type="hidden" name="id" value={category.id}/><input name="color" type="color" defaultValue={category.color} disabled={locked}/><input name="name" required minLength={2} maxLength={80} defaultValue={category.name} disabled={locked}/><Button type="submit" disabled={saving||locked}><Save size={14}/> Simpan</Button><Button variant="secondary" onClick={()=>setEditingCategoryId(null)}>Batal</Button></form>:<div className={`expense-category-item ${category.active?'':'inactive'}`} key={category.id}><i style={{background:category.color}}/><span><strong>{category.name}</strong><small>{category.transactionCount||0} transaksi · {category.budgetCount||0} pos RAB{(category.transactionCount||0)+(category.budgetCount||0)===0?' · dapat dihapus':!category.active?' · disimpan untuk histori':''}</small></span><Badge tone={category.active?'success':'neutral'}>{category.active?'Aktif':'Nonaktif'}</Badge>{!locked&&<><button aria-label={`Edit ${category.name}`} onClick={()=>setEditingCategoryId(category.id)}><Pencil size={15}/></button><button className="category-delete" aria-label={`Hapus ${category.name}`} title={(category.transactionCount||0)+(category.budgetCount||0)>0?'Kategori sudah digunakan dan hanya dapat dinonaktifkan':'Hapus kategori'} disabled={saving||(category.transactionCount||0)+(category.budgetCount||0)>0} onClick={()=>void deleteExpenseCategory(category)}><Trash2 size={15}/></button><button className="category-toggle" onClick={()=>void toggleExpenseCategory(category)}>{category.active?'Nonaktifkan':'Aktifkan'}</button></>}</div>)}</div>
