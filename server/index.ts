...
app.get('/api/bootstrap', requireAuth, async (req: AuthedRequest, res) => {
  ...
  const [organization, accounts, transactions, depositActivities, bills, purchases, settings] = await Promise.all([
    ...
    coalesce((select sum(-de.amount) from transaction_entries de join transactions dt on dt.id=de.transaction_id where de.account_id=a.id and dt.status='posted' and dt.kind='deposit_usage' and not exists(select 1 from transactions reversal where reversal.reversal_of=dt.id and reversal.status='posted') and date_trunc('month',dt.transaction_date)=date_trunc('month',current_date)),0)::numeric "monthlyUsage",
    coalesce((select sum(-de.amount)/30 from transaction_entries de join transactions dt on dt.id=de.transaction_id where de.account_id=a.id and dt.status='posted' and dt.kind='deposit_usage' and not exists(select 1 from transactions reversal where reversal.reversal_of=dt.id and reversal.status='posted') and dt.transaction_date>current_date-interval '30 days'),0)::numeric "dailyAverage"
